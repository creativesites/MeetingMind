...
object TranscriptChunker {
    ...
    // itself — reserve room for both before deciding how much transcript text fits. Exposed as
    // the default for [chunk]'s optional overhead parameters — a caller with a smaller, terser
    // prompt (e.g. transcript cleanup, see [com.example.ai.pipeline.TranscriptAiCleanupEngine])
    // passes its own real numbers instead of inheriting extraction's larger schema overhead.
    const val RESERVED_PROMPT_OVERHEAD_TOKENS = 700
    const val RESERVED_OUTPUT_TOKENS = 700
    ...
    fun chunk(
        ...
        reservedPromptOverheadTokens: Int = RESERVED_PROMPT_OVERHEAD_TOKENS,
        reservedOutputTokens: Int = RESERVED_OUTPUT_TOKENS
    ): List<TranscriptChunk> {
        if (segments.isEmpty()) return emptyList()
        val budgetTokens = (contextLengthTokens - reservedPromptOverheadTokens - reservedOutputTokens).coerceAtLeast(256)
