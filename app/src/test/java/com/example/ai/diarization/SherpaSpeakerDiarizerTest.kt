 ...
 * 2. The pure post-processing over raw (start, end, speakerIndex) output — clustering
 *    configuration and the two noise filters — has no sherpa-onnx dependency whatsoever and is
 *    fully unit-testable with synthetic data. Real acoustic diarization accuracy itself is NOT
 *    covered here — see docs/AI_ARCHITECTURE.md "Known Limitations" for what remains device-only.
 */
...
class SherpaSpeakerDiarizerTest {
    ...
    fun `reports model unavailable when no diarization model is installed`() = runBlocking {
        ...
        val result = diarizer.diarize(File(context.cacheDir, "does-not-matter.m4a"), 5000L, "m1")

        assertTrue(result is AiResult.ModelUnavailable)
    }

    // Word -> speaker attribution used to live here as `reconcileTranscriptWithSpeakers`, which
    // gave every word of an ASR segment to whichever speaker overlapped that segment the most.
    // It now lives one layer down, per word and with a confidence — see WordSpeakerAttributorTest.
