...
**Deferred to Stage 3+**: markers (§3.4) and quick notes (§3.5) as extraction-hint capture data are Stage 3 (`#7a`/`#7b` screens), not this one; deferred processing via `MeetingStatus.SAVED`/`QUEUED` is scaffolded (the enum values exist) but not yet wired in — `stopRecording` still persists the row as `MeetingStatus.RECORDING` (via the pre-existing `createInitialMeeting`, unchanged by this phase) and the caller still navigates straight to `ProcessingScreen`, which drives it to `PROCESSING` immediately; nothing yet lands on `SAVED`/`QUEUED` for a user-chosen "process later" path — that's Stage 4.

## 4f. Crash Recovery Journal + Storage/Battery Warnings (Phase 15 §Part 2 §3.7-§3.8)

**Journal**: `RecordingJournalStore` (`core/audio/RecordingJournal.kt`) writes a plain `recovery.json` in `filesDir` — not Room, since `MeetingRecordingService` must be able to write and read it with no database machinery required, and may be the only component alive when the process dies. Every write goes to a `.tmp` file first, then renames over the real one, so a process death mid-write never leaves a half-written journal that could misreport what happened; a journal `read()` can't parse is treated as no journal at all rather than crashing app startup. `MeetingRecordingService` writes it on every state transition (start/pause/resume) and at least every 10 seconds while `RECORDING`/`PAUSED` (`startMaintenanceLoop`), and clears it on every clean stop (`stopRecording`, right after the Meeting row is persisted) or discard. Markers/notes fields are always written as zero — real counts don't exist yet (Stage 3), so the journal never fabricates them.

**Recovery on next launch**: `MainActivity`'s `MeetMindApp()` reads the journal once via `LaunchedEffect(Unit)`. A journal whose `state` is anything other than `RECORDING`/`PAUSED` (or no journal at all) means the last capture stopped cleanly — nothing to show. Finding one of those two states is itself the crash signal (a clean stop always clears the journal first), and surfaces a mandatory `AlertDialog`: **Save recording · Delete**. Per spec ("never auto-delete, never silently discard, never overwrite on the next session"), the back gesture only dismisses this composition's local state — the journal itself is untouched, so the same prompt reappears on the next app launch until the user picks one of the two real actions.

**"Keep recording" (resuming capture) is deliberately not offered.** The spec's copy names three options, but seamlessly continuing a recording after the process that held its `MediaRecorder` session died is exactly what §3.3's "segments" architecture is for — deferred to Stage 6. Offering a "Keep recording" button that actually just started an unrelated new recording would be the dishonest-UI pattern this whole rewrite exists to avoid, so this pass only offers the two options it can build for real.

**"Save recording" reuses the exact same path a normal finished recording takes** — `createInitialMeeting` with the journal's audio path and type, then `navController.navigate(Routes.processingRoute(...))` with the journal's `lastKnownDurationMs` — rather than inventing separate recovery-specific persistence logic. One known, real limitation worth stating plainly: `AudioRecorder` records to an MPEG-4 (`.m4a`) container via `MediaRecorder`, whose muxer normally writes the file's index (`moov` atom) only when `stop()` is called. A crash means `stop()` never ran, so a recovered file's container may be incomplete even though the audio data itself is on disk. This pass does not attempt MP4 repair — building a real repair path (or switching capture to a crash-safe container) is future work, not something to silently claim works. What already exists and this leans on: `SherpaParakeetSpeechRecognizer`'s decode path already fails through the existing `AiResult` error channel on a bad file rather than crashing, so a genuinely unplayable recovered file surfaces the same "couldn't process this" state the app already has for any other bad audio input — it does not corrupt app state or crash.

**Storage/battery warnings (§3.8)**: `RecordingCapacity` (`core/audio/RecordingCapacity.kt`) is a pure-function object over primitives (no `Context`), directly unit-tested without Robolectric — `formatStorageLine`, `shouldRefuseToStart` (200 MB floor), and `inFlightWarning` (500 MB / 15% thresholds, storage checked first). Pre-flight: the existing `RecordingTypePickerScreen` (already effectively the `#7a` screen, just not yet restyled — that's Stage 3) now shows the storage line and disables both Start buttons below the floor, computed once via `DeviceCapabilityDetector.getAvailableStorageMb()` on screen entry. In-flight: `MeetingRecordingService`'s maintenance loop re-evaluates every 10 seconds via `DeviceCapabilityDetector` and a sticky-broadcast battery read (`Intent.ACTION_BATTERY_CHANGED`, no persistent `BroadcastReceiver` needed for a value read this infrequently), publishing the result as `capacityWarning: StateFlow<String?>` — read by `RecordingViewModel`/`RecordingScreen` as a single quiet line and folded into the foreground notification's text, never a modal over a live recording.
